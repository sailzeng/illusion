									...
									QStringList &error_tips)
{
	...
	if (!file_info.exists())
	{
		error_tips.append(QString::fromLocal8Bit("EXCEL文件[%1]并不存在，请检查。").
			arg(excel_file));
		return -1;
	}
...
int Biko_Read_Config::read_excel(QStringList &error_tips)
{
	//
	int ret = 0;
									  ...
									  QStringList & tips_info)
{
	...
	for (int i = 0; i < msg_count; ++i)
	{
		...
		if (table_msg_desc && table_msg_desc->field_count() > 0 )
		{
			...
			ils_ptr.release();
			auto iter = proto_cfg_map_.find(proto_fname);
			if (iter == proto_cfg_map_.end())
			{
